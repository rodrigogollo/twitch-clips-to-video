    ...
    { clipList ? 
          clipList.map((clip:any, i:number, origin:any) => {
            ...
            if(i > 0) {
              ...
              transitionFrom += Math.round(origin.slice(0, i).reduce((acc:number, obj:any) => (acc + obj.data.duration), 0) * 30) + transition * i;
            }
            return (
              <div key={clip.data.id + "_div"}>
                ...
                <Sequence key={clip.data.id + "_seq_clip"} from={duration + transition} durationInFrames={Math.round(clip.data.duration * 30)}>
