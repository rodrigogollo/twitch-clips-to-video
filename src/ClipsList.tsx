            ...
            return (
              <div key={clip.data.id + "_div"}>
                {/* <Sequence key={clip.data.id + "_seq_info"} from={transitionFrom}  durationInFrames={transition}>
                  <ClipInfo key={clip.data.id + "_info"} broadcaster={clip.data.broadcaster_name} title={clip.data.title} 
                    date={clip.data.created_at.substring(0, 10)} creator={clip.data.creator_name}/>
                </Sequence>   */}
                <Sequence key={clip.data.id + "_seq_clip"} from={duration} durationInFrames={Math.round(clip.data.duration * 60)}>
